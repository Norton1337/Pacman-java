...
public class OrangeGhost extends Creature{
	...
	//Animation
		...
		private int orangeDotsCounter = 0;
		


		private int dotsLimit=60;
		
		

	public OrangeGhost(Handler handler, float x, float y, int width, int height) {
		...
		//Animations
		animRight = new Animation(100,Assets.orangeGhostR);
		animLeft = new Animation(100,Assets.orangeGhostL);
		animUp = new Animation(100,Assets.orangeGhostU);
		animDown = new Animation(100,Assets.orangeGhostD);
	...
	public int getOrangeDotsCounter() {
		return orangeDotsCounter;
	}

	public void setOrangeDotsCounter(int orangeDotsCounter) {
		this.orangeDotsCounter = orangeDotsCounter;
	}
	...
	public float closestPathValue(float xCoord, float yCoord) {
		double ifUp = 9999999;
		...
		if(goUp) {
			number  = (int) (Math.abs(xCoord)-Math.abs(this.bounds.x));
			number2 = (int) (Math.abs(yCoord)-Math.abs(this.bounds.y-1)); 
			ifUp = ((Math.pow(number,2)) + (Math.pow(number2,2)));
			//System.out.println("up: "+ ifUp);
		} 
		if(goLeft) {
			number  = (int) (Math.abs(xCoord)-Math.abs(this.bounds.x-1));
			number2 = (int) (Math.abs(yCoord)-Math.abs(this.bounds.y));
